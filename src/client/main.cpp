...
int main(int argc, char *argv[])
{
    ...
    for (;;)
    {
            ...
            cin.getline(password, 50); // 使用getline读取密码，允许密码中包含空格
            {
                ...
                else
                {
                    ...
                    if (n > 0)
                    {
                        ...
                        {
                            cout << "登录失败，错误信息：" << response["errmsg"].get<string>() << endl;
                        }
                    }
                    else
                    {
        ...
        case 2: // 注册业务
            ...
            cout << "请输入用户密码：";
            cin.getline(password1, 50); // 使用getline读取密码，允许密码中包含空格
            {
                json js;
                js["msgid"] = REG_MSG;
                js["name"] = name;
                js["password"] = password1;
                string request = js.dump();
                int len = send(clientfd, request.c_str(), request.size(), 0);
                if (len == -1)
                {
